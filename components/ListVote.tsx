	...
	return (
		<div className=" w-full mx-auto py-10 px-10 md:p-5 grid  grid-cols-1 md:grid-cols-2 xl:grid-cols-3 grid-flow-dense gap-y-20 gap-10 ">
			{votes?.map(({ title, end_date, users, id }, index) => {
				...
				return (
					<Link
						href={"/vote/" + id + (isExpire ? "/true" : "")}
						key={index}
						className=" w-full md:w-96  "
					>
						<div className="relative group">
							<div className=" border border-zinc-600 rounded-md p-5 space-y-3 bg-green-600 group-hover:translate-x-3 group-hover:translate-y-3 transition-all relative">
